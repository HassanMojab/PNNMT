...
for k in list_of_tasks:
    ...
    train_dataloaders[k] = DataLoader(
        train_corpus[k], batch_size=batch_size, shuffle=True, pin_memory=True
    )
    dev_dataloaders[k] = DataLoader(
        dev_corpus[k], batch_size=batch_size, pin_memory=True
    )
...
class Sampler:
    ...
    def __next__(self):
        curr_p = self.func(
            self.init_p, self.list_of_tasks, self.curr_step, self.total_steps
        )
        self.curr_step += 1
        ...
        l = {
            "task": tasks[0],
            "data": get_batch(
                self.list_of_iters[tasks[0]], train_dataloaders[tasks[0]]
            ),
        }
        return l


def identity(x, y, z, w):
    return x
...
def UniformBatchSampler():
    p = np.array(
        [
            len(train_dataloaders[y])
            * 1.0
            / sum([len(train_dataloaders[x]) for x in list_of_tasks])
